...
# -*- coding: utf-8 -*-
"""
Created on Tue Aug  8 13:58:12 2017
...
def cnn_model_fn(features, labels, mode):
  ...
  # Input Layer
  # Reshape X to 5-D tensor: [batch_size, depth, width, height, channels]
  # particle images are 41*41*41 pixels, and have one color channel
  input_layer = tf.reshape(features, [-1, 41, 41, 41, 1])
  ...
  pool1 = tf.layers.max_pooling3d(inputs=conv1, pool_size=[3, 2, 2], strides=2, padding="valid")
  ...
  pool2 = tf.layers.max_pooling3d(inputs=conv2, pool_size=[2, 3, 3], strides=2, padding="same")
...
def main(unused_argv):
  ...
  train_data = plt.imread('pixel train.txt')  # Returns np.array
  train_data = np.asarray(train_data, dtype=np.float32)
  
  train_labels = np.loadtxt('pixel label.txt')
  train_labels = np.asarray(train_labels, dtype=np.int32)
  
  
  eval_data = plt.imread('sada')  # Returns np.array
  eval_data = np.asarray(eval_data, dtype=np.float32)
  
  eval_labels = np.loadtxt('pixel label.txt')

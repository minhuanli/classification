# -*- coding: utf-8 -*-
"""
Created on Fri Jan 19 17:06:22 2018
...
#feature = 87
droprate = 0.3
model_dir = r'D:\liminhuan\machine learning\20180119\NN2checkpoint'
####################################
...
#######################  I . load data part  ####################################
eval_data = plt.imread(r'D:\liminhuan\machine learning\20180119\sboxpad22.tif')
...
input_layer = tf.reshape(xs, [-1, 21, 31, 31, 1])
...
# Input Tensor Shape: [batch_size, 21, 31, 31, 1]
...
conv1 = tf.layers.conv3d(
    ...
    strides=[2,3,3],
    padding="valid",
    activation=tf.nn.relu,name ='7c3layer3l2')
# Pooling Layer #1
# First max pooling layer with a 2*2*2 filter and stride of 2 
# Input Tensor Shape: [batch_size, 9, 9, 9, 32]
# Output Tensor Shape: [batch_size, 4, 4, 4, 32]
pool1 = tf.layers.max_pooling3d(inputs=conv1, pool_size=[2, 2, 2], strides=2, padding="valid",name='7player4l2')

# Convolutional Layer #2
# Computes 32 features using a 3*3*3 filter.
# kernel size here correspond to the first neighbor shell
# Padding is added to preserve width and height.
# Input Tensor Shape: [batch_size, 4, 4, 4, 32]
# Output Tensor Shape: [batch_size, 4, 4, 4, 32]
conv2 = tf.layers.conv3d(
    ...
    kernel_size=[2,2,2],
    padding="same",
    activation=tf.nn.relu,name='7c3layer5l2')

# Pooling Layer #2
# Second max pooling layer with a 2*2*2 filter and stride of 2
# Input Tensor Shape: [batch_size, 4, 4, 4, 32]
# Output Tensor Shape: [batch_size, 2, 2, 2, 32]
pool2 = tf.layers.max_pooling3d(inputs=conv2, pool_size=[2, 2, 2], strides=2, padding="same",name='7player6l2')


# Flatten tensor into a batch of vectors
# Input Tensor Shape: [batch_size, 5, 5, 5, 32]
# Output Tensor Shape: [batch_size, 5 * 5 * 5 * 32]
pool2_flat = tf.reshape(pool2, [-1, 2 * 2 * 2 * 32])

# Dense Layer
# Densely connected layer with 1024 neurons
# Input Tensor Shape: [batch_size, 2 * 2 * 2 * 32]
# Output Tensor Shape: [batch_size, 256]
dense = tf.layers.dense(inputs=pool2_flat, units=256, activation=tf.nn.relu,name='7dlayer7l2')
...
logits = tf.layers.dense(inputs=dropout, units=classnum,name='7logitlayer8l2')
